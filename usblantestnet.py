...
def disable_enable_device(tests_count, short_id, action, startTime):
    ...
    logging.info(f"[{tests_count}:{elapsed:.0f}] {short_id}: {action.title()}d")
    return output
...
def ping_device(ip, timeout=0.5, count=1, tests_count=None):
    try:
...
def wget_device(tests_count, get_count, ip, iface, startTime, networkStartTime, timeout=2, count=None ):
    try:
        ...
        if r.status_code == 200:
            logging.info(f"[{tests_count}:{time()-startTime:.0f}:{time()-networkStartTime:.0f}] {iface}: HTTP GET success: {url} [{count}]")
            return True, r.text[:200]
        else:
            logging.warning(f"[{tests_count}:{time()-startTime:.0f}:{time()-networkStartTime:.0f}] {iface}: HTTP GET failed: status {r.status_code} [{count}]")
    except Exception as e:
...
def main():
    ...
    net_ok = net_notok = ping_ok = ping_notok = get_ok = get_notok = 0
    ...
    while True:
        ...
        while True:
            ...
            if (not successFlag and args.wait_on_error) or args.no_auto_replug:
                logging.info(f"[{tests_count}:{time()-startTime:.0f}] {short_id}: REPLUG DEVICE NOW")
                waitCount = 2000
            else:
            ...
            for check in range(waitCount):
                ...
                if not is_device_enabled(tests_count, short_id, startTime, args.wait_on_error ):
                    logging.info(f"[{tests_count}:{time()-startTime:.0f}] {short_id}: USB Device Disconnected [{check+1}]")
                    disabled_ok += 1
                    break
            else:
                disabled_notok += 1
                logging.warning(f"[{tests_count}:{time()-startTime:.0f}] {short_id}: USB Device did not disable after 10s.")
                break
            ...
            for attempt in range(20 if not args.no_auto_replug else 2000):
                ...
                if is_device_enabled(tests_count, short_id, startTime, args.wait_on_error):
                    enabled_ok += 1
                    logging.info(f"[{tests_count}:{time()-startTime:.0f}] {short_id}: USB Device Connected [{attempt+1}]")
                    break
            else:
                enabled_notok += 1
                logging.error(f"{tests_count}:{time()-startTime:.0f}] {short_id}: USB Device did not Connected after 10 seconds.")
                break
            ...
            for i in range(4):
                ...
                if stats:
                    logging.info(f"[{tests_count}:{time()-startTime:.0f}] {iface}: {args.ip} " +
                    f"activity: {bytes_sent}:{bytes_recv} {packets_sent}:{packets_recv} {dropped_sent}:{dropped_recv} [{i+1}]")
                    net_ok += 1
                    break
            else:
                net_notok += 1
                logging.error(f"[{tests_count}:{time()-startTime:.0f}] Network not found for {args.ip}.")
                break
            ...
            startPingTime = time()
            for i in range(120):
                if ping_device(args.ip, timeout=0.5, count=1, tests_count=tests_count):
                    logging.info(f"[{tests_count}:{time()-startTime:.0f}:{time()-networkStartTime:.0f}] {iface}: Ping {args.ip} [{i+1}]")
                    # Do two extra for confirmation
                    ping_device(args.ip, timeout=0.5, count=1)
                    ping_device(args.ip, timeout=0.5, count=1)
                    ping_ok += 1
                    break
                sleep(0.5)
                ...
                ping_notok += 1
                logging.error(f"[{tests_count}:{time()-startTime:.0f}:{time()-networkStartTime:.0f}] {iface} Ping to {args.ip} failed.")
                sleep(10)
                break
            ...
            if not args.no_get:
                for i in range(1,3):
                    ...
                    get_notok += 1
                    logging.error(f"[{tests_count}:{time()-startTime:.0f}] HTTP GET failed after 3 tries.")
                    break
        ...
        logging_level(f"[{tests_count}:{time()-startTime:.0f}:{time()-networkStartTime:.0f}] {'OK' if successFlag else 'NOTOK'} " +
            f"disable: {disabled_ok}:{disabled_notok} " +
            f"enable: {enabled_ok}:{enabled_notok} " +
            f"net: {net_ok}:{net_notok} " +
            f"ping: {ping_ok}:{ping_notok} " +
